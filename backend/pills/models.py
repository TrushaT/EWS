...
class Medical(models.Model):
    user = models.ForeignKey(User,related_name='medical',on_delete=models.CASCADE)
    alergies = models.CharField(max_length=100)

class Pills(models.Model):
    user = models.ForeignKey(User,related_name='pills', on_delete=models.CASCADE)
    pills = models.CharField(max_length=100)
    exist = models.BooleanField(default=True)
    count = models.IntegerField(default=0)

class Schedule(models.Model):
    pills = models.ForeignKey(Pills,related_name='schedule', on_delete=models.CASCADE)
